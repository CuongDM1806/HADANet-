...
for num in range(17, 18):
    ...
    # 从左手特征文件夹和标签文件夹加载数据
    feature_folder = "../cho_feature_data\left_feature_DE/"
    label_folder = "../cho_feature_data\left_label/"
    ...
    #
    # 从右手特征文件夹和标签文件夹加载数据
    feature_folder = "../cho_feature_data/right_feature_DE/"
    label_folder = "../cho_feature_data/right_label/"
...
#     print(labels.shape)  # torch.Size([90])
    ...
    for epoch in range(epochs):
        ...
        # 在计算Recall和F1-Score之前，确保张量在CPU上
        y_test_cpu = y_test.cpu().numpy()
        predictions_cpu = predictions.cpu().numpy()

        # 计算Recall
        recall = recall_score(y_test_cpu, predictions_cpu, average='macro')
        print(f"Recall: {recall}")

        # 计算F1-Score
        f1 = f1_score(y_test_cpu, predictions_cpu, average='macro')  # 计算宏平均 F1-Score
