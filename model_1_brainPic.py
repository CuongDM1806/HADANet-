...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # 前向传播过程
        print(x.shape)  # torch.Size([15, 1280])
...
class DomainAdaptationModel(nn.Module):
    ...
    def forward(self, source_data, target_data):
        ...
        source_classification, source_barin = self.mlp(source_features)
        # target_classification, target_brain = self.mlp(target_features)
        #
        return source_classification, source_barin #domain_loss
...
def train(model, train_loader, optimizer, criterion, domain_criterion, device):
    ...
    for source_data, target_data, source_labels in train_loader:
        ...
        # 正向传播
        source_classification, source_barin = model(source_data, target_data) #domain_loss
...
def predict(model, test_data, device):
    ...
    with torch.no_grad():
        ...
        output, brain = model.mlp(features)
        _, predicted = torch.max(output, 1)
        return predicted, brain
...
for num in range(29, 30):
    ...
    for epoch in range(epochs):
        ...
        predictions, brain = predict(model, x_test_tensor, device)
...
file_name = "physionet_{}_brain_model.pth".format(num)
